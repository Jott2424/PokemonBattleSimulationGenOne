"""
Queues battles for Yellow-only trainer matchups. Every (trainer1, trainer2,
profile1, profile2) matchup gets a flat target of SAMPLES_PER_COMBO (10)
simulations — no decisiveness evaluation.

Sibling of populate_battles.py (which does the same for Blue). Kept as a
separate script so the Blue queuing path stays byte-for-byte unchanged. The
two are safe to run in either order and any number of times: the NOT EXISTS
guard and the idx_battles_to_sim_combo_sample unique index are keyed on
trainer ids, and Yellow trainers (fk_games_id = 3) never collide with Blue
ones (fk_games_id = 2), so this only ever appends new Yellow rows — existing
Blue rows in battles_to_sim and every downstream result table are untouched.
Results land in the same shared tables (sim_battles, sim_battle_turns, …) via
scripts/main.py, appended alongside the Blue battles.
...
Usage (run from output/):
    python scripts/populate_battles_yellow.py
"""
import json
import os
...
def main():
    ...
    with conn:
        with conn.cursor() as cur:
            cur.execute(SQL, {"yellow": YELLOW_GAME_ID, "samples": SAMPLES_PER_COMBO, "limit": limit})

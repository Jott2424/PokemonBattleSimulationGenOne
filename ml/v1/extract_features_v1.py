...
Usage (run from ml/v1/):
    python extract_features_v1.py [--out FILE] [--games-id N]
"""
import argparse
import json
import os
...
def _load_moves(conn):
    return pd.read_sql(
        "SELECT pk_moves_id AS move_id, fk_types_id AS move_type, "
...
def build_side_profile(slots: pd.DataFrame, moves: pd.DataFrame, type_names: list) -> pd.DataFrame:
    ...
    for c in move_type_cols + stab_type_cols + ["move_count", "physical_move_count",
            "special_move_count", "status_move_count", "stab_move_count", "total_pp"]:
...
def build_move_table(slots: pd.DataFrame, moves_ref: pd.DataFrame) -> pd.DataFrame:
    ...
    with move_type/category/power/accuracy/pp resolved and an is_stab flag computed
    against that Pokemon's own species type(s)."""
    key = ["battle_id", "trainer_id"]
    frames = []
    for i in range(1, 5):
        col = f"move{i}_id"
